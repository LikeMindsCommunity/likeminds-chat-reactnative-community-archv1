...
export default React.forwardRef(
  <T extends any>(
    ...
    forwardedRef:
      | ((instance: FlashList<T> | null) => void)
      | MutableRefObject<FlashList<T> | null>
      | null,
  ) => {
    const flRef = useRef<FlashList<T> | null>(null);
    ...
    const enableMvcpWithRetriesRef = useRef(() => {
      ...
      debounceTimeoutId.current = setTimeout(async () => {
        ...
        try {
          const _handle: number =
            await ScrollViewManager.enableMaintainVisibleContentPosition(
              scrollableNode,
    ...
    const refCallback = useRef<(instance: FlashList<T> | null) => void>(ref => {
      flRef.current = ref;
      enableMvcpWithRetriesRef.current();
    ...
    return <FlashList<T> {...props} ref={refCallback} />;
  },
